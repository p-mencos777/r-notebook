...
counties_selected <- counties %>%
  select(state, county, pupulation, private_work, public_work, self_employed)

counties_selected %>%
  arrange(desc(public_work))

counties_selected <- counties %>%
  selected(state, county, population)
...
counties_selected %>%
  summarize(min_population=min(population=min(population), max_unemployment=max(unemployment), averagen_income=mean(income))
```
Summarizing by state

```{r}
counties_selected <- counties %>%
  select(state, count, population, land_area)

counties_selected %>%
  group_by(state) %>%
  summarize(total_area=sum(land_area), total_populatinon=sum(population)) %>%
